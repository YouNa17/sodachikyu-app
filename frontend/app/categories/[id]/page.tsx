...
export default function ActionListPage() {
  ...
  const [selectedAction, setSelectedAction] = useState<{ title: string; detail: string } | null>(null);
    ...
    <main style={{ padding: "20px", maxWidth: "500px", margin: "0 auto", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gap: "15px" }}>
        {actions.length === 0 ? (
          ...
          actions.map((action) => (
            <button key={action.id} onClick={() => setSelectedAction(action)} style={{ padding: "20px", borderRadius: "16px", border: "1px solid #ddd", backgroundColor: "white", textAlign: "left", cursor: "pointer" }}>
      ...
      {selectedAction && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.7)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000, padding: "20px" }}>
          <div style={{ backgroundColor: "white", padding: "30px", borderRadius: "28px", width: "100%", maxWidth: "400px", textAlign: "center" }}>
            ...
            <button onClick={() => setSelectedAction(null)} style={{ marginTop: "15px", background: "none", border: "none", color: "#999", cursor: "pointer" }}>
              とじる
            </button>
          </div>
        </div>
